...
namespace BayatGames.SaveGameFree
{
	...
	public static class SaveGame
	{
		...
		/// Load handler.
		/// </summary>
		public delegate void LoadHandler(object loadedObj, string identifier, bool encode, string password, ISaveGameSerializer serializer, ISaveGameEncoder encoder, Encoding encoding,
			SaveGamePath path);
			...
			if (!IsFilePath(identifier))
			{
				switch (path)
				{
					...
					case SaveGamePath.PersistentDataPath:
						filePath = string.Format("{0}/{1}", Application.persistentDataPath + "/SaveGame", identifier);
						break;
					case SaveGamePath.DataPath:
						filePath = string.Format("{0}/{1}", Application.dataPath + "/SaveGame", identifier);
						break;
				}
			}
			else
		...
		public static void DeleteAll(SaveGamePath path)
		{
			...
			{
				case SaveGamePath.PersistentDataPath:
					dirPath = Application.persistentDataPath + "/SaveGame";
					break;
				case SaveGamePath.DataPath:
