...
public class Replay_System {
        private readonly WaitForFixedUpdate _wait = new WaitForFixedUpdate();
        private IRecordDependency AnimController;

        public Replay_System(MonoBehaviour runner, IRecordDependency anim = null)
        {
            AnimController = anim;
        ...
        private IEnumerator FixedUpdate() {
            while (true) {
                yield return _wait;
                if(AnimController == null || (AnimController != null && !AnimController.PauseRecorder))
                {
                    AddSnapshot();
        ...
        private void AddSnapshot() {
            ...
            {
                if(AnimController == null) _currentRun.AddSnapshot(_elapsedRecordingTime);
                else _currentRun.AddSnapshot(_elapsedRecordingTime, AnimController.GetCurrentAnimation);
            }
        ...
        private void UpdateReplay() {
            ...
            if(AnimController != null) AnimController.PlayAnimation(_currentReplay.EvaluateAnim(_replaySmoothedTime));
